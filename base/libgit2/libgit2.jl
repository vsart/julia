...
"""
    LibGit2.isdirty(repo::GitRepo, paths::AbstractString=""; cached::Bool=false) -> Bool

Checks if there have been any changes to tracked files in the working tree (if
`cached=false`) or the index (if `cached=true`).
`paths` are the specifications for options for the diff.
...
"""
    LibGit2.isdiff(repo::GitRepo, treeish::AbstractString, paths::AbstractString=""; cached::Bool=false)
...
`b`, a [`GitHash`](@ref) in string form. `a` and `b` could be branches, tags,
or commits.
"""
...
Pushes updates to the `remote` upstream of `repo`, looking at `remoteurl`.
...
`true`, as the branch creation finishes the branch head will be set as the HEAD of `repo`.
"""
