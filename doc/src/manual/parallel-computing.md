...
by the [`ParallelAccumulator`](@ref) with final reductions performed on the caller.
...
[`ParallelAccumulator`](@ref)s can be used to get around this limitation:
...
for the parallel computation. In case of `@parallel for` used in conjunction with [`ParallelAccumulator`](@ref)s,
